...
data:
  ...
  bundledCode: "#line 2 \"modulo/mod-kth-root.hpp\"\n\n#line 2 \"internal/internal-math.hpp\"\
    ...
    \ 1 ? a : -1;\n  if (a <= 1 || k <= 1) return a;\n  assert(p > 2);\n  if (mint::get_mod()\
    \ != decltype(mint::a)(p)) mint::set_mod(p);\n  INT g = gcd(p - 1, k);\n  if (nyaan_internal::modpow<INT,\
    \ LINT>(a, (p - 1) / g, p) != 1) return -1;\n  a = mint(a).pow(nyaan_internal::inv(k\
...
$g$の素因数分解も[Miller-RabinとPollardの$\rho$法](https://nyaannyaan.github.io/library/prime/fast-factorize.hpp)を利用すれば$\mathrm{O}(g^{\frac{1}{4}})$で計算できるので、全体の計算量は$\mathrm{O}(\min (p,k)^{\frac{1}{4}})$となる。
