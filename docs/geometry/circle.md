...
- `pair<Point, Point> crosspoint(const Circle& c1, const Circle& c2)`: 円 `c1` と `c2` の交点 2 点を返す関数。必ず 2 点の組を返すが、`intersect(c1, c2)` の返り値によっては 2 点が異なることや 2 点が実際に円 `c1` と `c2` の交点であるとは限らないことに注意。
